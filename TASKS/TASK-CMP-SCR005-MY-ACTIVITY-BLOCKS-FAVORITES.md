---
task_id: CMP-SCR005-MY-ACTIVITY-BLOCKS-FAVORITES
type: component
screen_id: SCR-005
route: "`/account`"
page_entry: "`src/components/scr005/BlocksFavoritesTab.tsx`"
depends_on:
  - API-MATES-ROUTES
...
# CMP-SCR005-MY-ACTIVITY-BLOCKS-FAVORITES — 차단 목록·즐겨찾기 탭

## Context
`CMP-SCR005-MY-ACTIVITY-BLOCKS-FAVORITES`(차단 목록·즐겨찾기 탭)은 SCR-005(`/account`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-005
- Route: `/account`
- Page Entry: `src/components/scr005/BlocksFavoritesTab.tsx`

## Design Ref
- D-001 §14(Empty — 완성형 문장), §9(즐겨찾기 카드)
- `design-reference/UI_CONTRACT.md` 5장(SCR-005) — 영역 순서·주요 Component·상태·사용자 행동·금지 기능
...
## Functional AC
- 차단 해제, 즐겨찾기 목록(localStorage)

## Visual AC
- 차단 0건 시 "아직 차단한 사용자가 없습니다" 완성형 Empty

## Security/Privacy AC
N/A

## Test Cases
- TC-1(Functional): 차단 해제, 즐겨찾기 목록(localStorage)
- TC-2(Visual): 차단 0건 시 "아직 차단한 사용자가 없습니다" 완성형 Empty
