---
task_id: CMP-SCR004-REPORT-BLOCK-ACTIONS
type: component
screen_id: SCR-004
route: "`/mates`"
page_entry: "`src/components/scr004/ReportBlockActions.tsx`"
depends_on:
  - API-MATES-ROUTES
...
# CMP-SCR004-REPORT-BLOCK-ACTIONS — 신고·차단 실행 UI

## Context
`CMP-SCR004-REPORT-BLOCK-ACTIONS`(신고·차단 실행 UI)은 SCR-004(`/mates`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-004
- Route: `/mates`
- Page Entry: `src/components/scr004/ReportBlockActions.tsx`

## Design Ref
- D-001 §13(Toast — 접수 결과 표시)
...
## Functional AC
- 사유 코드+설명 신고, 차단/해제, 접수번호 Toast 표시
...
## Test Cases
- TC-1(Functional): 사유 코드+설명 신고, 차단/해제, 접수번호 Toast 표시
- TC-3(Security/Privacy): 차단 후 상호 노출 제한
