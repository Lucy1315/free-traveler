...
# SHARED-SHARE-UTIL — URL 공유 유틸(Web Share API+복사 폴백)

## Context
`SHARED-SHARE-UTIL`(URL 공유 유틸(Web Share API+복사 폴백))은 SCR-001,004 영역의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-001,004
- Route: N/A
- Page Entry: `src/lib/share.ts`

## Design Ref
- 시각 규격 없음(비가시 유틸) — 공유 대상은 §9 Destination Card, §11 Mate Post Card
...
## Test Cases
- TC-1(Functional): Web Share 실패 시 URL 복사 폴백
