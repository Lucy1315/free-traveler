...
# CMP-SCR004-INTRO-CTA — 동행 조회 Intro+작성 CTA

## Context
`CMP-SCR004-INTRO-CTA`(동행 조회 Intro+작성 CTA)은 SCR-004(`/mates`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.

## Project Scope
Impl. Status = **IMPLEMENT**(`docs/PROJECT_SCOPE.md`·`docs/06_SRS_UIUX_REVISED.md` 기준).

## Requirement Ref
해당 없음(N/A) — 이 Task는 특정 REQ ID에 직접 매핑되지 않는다(테스트 픽스처 또는 화면 안내 보조 Task).

## Screen / Route / Page Entry
- Screen: SCR-004
- Route: `/mates`
- Page Entry: `src/components/scr004/IntroCta.tsx`

## Design Ref
- D-001 §16(Section 계층 — Intro+CTA)
...
## Test Cases
- TC-1(Functional): 목적 설명+"새 모집글 작성" 버튼(→ SCR-003)
