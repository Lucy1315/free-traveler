...
type: test
page_entry: "`tests/unit/mateState.test.ts`"
depends_on:
  - DB-ACCESS
  - API-MATES-ROUTES
...
# UNIT-MATE-STATE — 참가/모집글 상태 전이 단위 테스트

## Context
`UNIT-MATE-STATE`(참가/모집글 상태 전이 단위 테스트)은 SCR-004,005 영역의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-004,005
- Route: N/A
- Page Entry: `tests/unit/mateState.test.ts`

## Design Ref
- 시각 검증 아님 — UI_CONTRACT 4장 상태 절 참조
...
## Functional AC
- PENDING→ACCEPTED/REJECTED, OPEN→CLOSED(종료일 계산), 중복요청 차단 테스트
...
## Test Cases
- TC-1(Functional): PENDING→ACCEPTED/REJECTED, OPEN→CLOSED(종료일 계산), 중복요청 차단 테스트
