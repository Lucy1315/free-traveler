...
## Context
`DB-SEED-BASE`(개발·테스트용 시드 데이터)은 SCR-004,005 영역의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
## Test Cases
- TC-1(Functional): 동행 모집글 샘플 5건, 회원 3건 시드
- TC-3(Security/Privacy): 연락처 패턴 없는 샘플 데이터만
