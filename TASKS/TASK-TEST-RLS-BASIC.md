...
# TEST-RLS-BASIC — RLS 기본 정책 테스트

## Context
`TEST-RLS-BASIC`(RLS 기본 정책 테스트)은 SCR-004,005 영역의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-004,005
- Route: N/A
- Page Entry: `tests/unit/rlsBasic.test.ts`

## Design Ref
- 시각 검증 아님 — REQ-FUNC-044/NF-013(서버 정책)
...
## Test Cases
- TC-1(Functional): 권한별 부정 접근 테스트(본인 아님/비로그인/Moderator 아님) 전부 403 또는 빈 결과
- TC-3(Security/Privacy): 비공개 데이터 열람 차단 확인
