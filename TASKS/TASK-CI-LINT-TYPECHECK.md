...
# CI-LINT-TYPECHECK — CI Lint·TypeScript strict 게이트

## Context
`CI-LINT-TYPECHECK`(CI Lint·TypeScript strict 게이트)은 ALL(5) 영역의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.

## Project Scope
Impl. Status = **IMPLEMENT**(`docs/PROJECT_SCOPE.md`·`docs/06_SRS_UIUX_REVISED.md` 기준).

## Requirement Ref
- **REQ-NF-031**: TypeScript strict·lint·unit test

## Screen / Route / Page Entry
- Screen: ALL(5)
- Route: N/A
- Page Entry: `.github/workflows/ci.yml`(또는 동등 CI 설정)
...
## Test Cases
- TC-1(Functional): `npm run lint`+`tsc --noEmit`+`npm test` main 병합 전 통과
- TC-4(Verify 연동): CI 실행 로그로 위 시나리오를 실행해 통과를 확인한다.
...
- [ ] Verify 절의 검증 수단(CI 실행 로그)을 실행했거나 실행 계획이 명시돼 있다.
