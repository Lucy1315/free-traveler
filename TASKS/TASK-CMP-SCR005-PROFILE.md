---
task_id: CMP-SCR005-PROFILE
type: component
screen_id: SCR-005
route: "`/account`"
page_entry: "`src/components/scr005/ProfileTab.tsx`"
depends_on:
  - DB-ACCESS
...
# CMP-SCR005-PROFILE — 프로필+성인확인 탭

## Context
`CMP-SCR005-PROFILE`(프로필+성인확인 탭)은 SCR-005(`/account`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-005
- Route: `/account`
- Page Entry: `src/components/scr005/ProfileTab.tsx`
...
## Test Cases
- TC-1(Functional): 닉네임/연령대/성별(선택)/여행스타일, 성인확인 배지
- TC-3(Security/Privacy): 정확한 생년월일 미저장(`is_adult`,`adult_verified_at`만)
