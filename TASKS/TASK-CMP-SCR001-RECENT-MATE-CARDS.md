...
# CMP-SCR001-RECENT-MATE-CARDS — 최근 동행글 카드 3개

## Context
`CMP-SCR001-RECENT-MATE-CARDS`(최근 동행글 카드 3개)은 SCR-001(`/`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-001
- Route: `/`
- Page Entry: `src/components/scr001/RecentMateCards.tsx`

## Design Ref
- D-001 §11(Mate Post Card)
...
## Test Cases
- TC-1(Functional): 최근 3건 표시, 0건 시 완성형 Empty(모집글 작성 CTA+이용 방법)
- TC-2(Visual): 연락처 미노출
