---
task_id: CMP-SCR005-AUTH-GUEST
type: component
screen_id: SCR-005
route: "`/account`"
page_entry: "`src/components/scr005/AuthGuest.tsx`"
depends_on:
  - DB-ACCESS
...
# CMP-SCR005-AUTH-GUEST — Guest 인증(로그인/가입/재설정)

## Context
`CMP-SCR005-AUTH-GUEST`(Guest 인증(로그인/가입/재설정))은 SCR-005(`/account`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-005
- Route: `/account`
- Page Entry: `src/components/scr005/AuthGuest.tsx`
...
## Test Cases
- TC-1(Functional): 이메일 가입·인증·로그인·로그아웃·비밀번호 재설정
- TC-3(Security/Privacy): Supabase Auth 세션 검증
