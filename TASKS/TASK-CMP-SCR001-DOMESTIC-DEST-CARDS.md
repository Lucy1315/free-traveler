---
task_id: CMP-SCR001-DOMESTIC-DEST-CARDS
type: component
screen_id: SCR-001
route: "`/`"
page_entry: "`src/components/scr001/DomesticDestinationGrid.tsx`"
depends_on:
...
# CMP-SCR001-DOMESTIC-DEST-CARDS — 국내 여행지 카드 목록(필터 포함)

## Context
`CMP-SCR001-DOMESTIC-DEST-CARDS`(국내 여행지 카드 목록(필터 포함))은 SCR-001(`/`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-001
- Route: `/`
- Page Entry: `src/components/scr001/DomesticDestinationGrid.tsx`
...
## Functional AC
- 국가/테마/계절/기간 필터, 카드 최소 6개, 0건 시 완성형 Empty(조건 초기화 버튼)

## Visual AC
- 14px radius 카드, 가격·별점 없음

## Security/Privacy AC
N/A

## Test Cases
- TC-1(Functional): 국가/테마/계절/기간 필터, 카드 최소 6개, 0건 시 완성형 Empty(조건 초기화 버튼)
- TC-2(Visual): 14px radius 카드, 가격·별점 없음
