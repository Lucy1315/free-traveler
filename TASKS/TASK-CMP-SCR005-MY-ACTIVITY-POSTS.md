...
# CMP-SCR005-MY-ACTIVITY-POSTS — 내 글·참가 요청 관리 탭

## Context
`CMP-SCR005-MY-ACTIVITY-POSTS`(내 글·참가 요청 관리 탭)은 SCR-005(`/account`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-005
- Route: `/account`
- Page Entry: `src/components/scr005/MyActivityPosts.tsx`

## Design Ref
- D-001 §11(Mate Post Card 재사용), §14(Empty)
...
## Visual AC
- 0건 시 완성형 Empty(작성 CTA)

## Security/Privacy AC
- 비작성자 변경 403

## Test Cases
- TC-1(Functional): 내 모집글 수정/마감/삭제, 참가 요청 승인/거절
- TC-2(Visual): 0건 시 완성형 Empty(작성 CTA)
- TC-3(Security/Privacy): 비작성자 변경 403
