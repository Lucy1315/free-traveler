...
# SHARED-TOAST-NOTIFICATIONS — Toast 알림 컴포넌트

## Context
`SHARED-TOAST-NOTIFICATIONS`(Toast 알림 컴포넌트)은 SCR-004,005 영역의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-004,005
- Route: N/A
- Page Entry: `src/components/ui/Toast.tsx`

## Design Ref
- D-001 §13(Alert·Toast)
...
## Visual AC
- D-001 §13 색상(success/error/info)

## Security/Privacy AC
- 실제 이메일 미발송 확인

## Test Cases
- TC-1(Functional): 성공/오류/정보 3종, 4초 자동 소멸, 이메일 미발송 대체 수단
- TC-2(Visual): D-001 §13 색상(success/error/info)
- TC-3(Security/Privacy): 실제 이메일 미발송 확인
