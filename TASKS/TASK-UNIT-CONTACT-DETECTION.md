...
type: test
screen_id: SCR-003
page_entry: "`tests/unit/contactDetection.test.ts`"
depends_on:
...
## Context
`UNIT-CONTACT-DETECTION`(연락처 패턴 탐지 단위 테스트)은 SCR-003 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-003
- Route: N/A
- Page Entry: `tests/unit/contactDetection.test.ts`

## Design Ref
- 시각 검증 아님 — UI_CONTRACT 3장 상태(연락처 패턴 탐지) 절 참조
...
## Functional AC
- 전화번호/이메일/메신저ID 패턴 샘플셋 탐지·오탐 테스트
...
## Test Cases
- TC-1(Functional): 전화번호/이메일/메신저ID 패턴 샘플셋 탐지·오탐 테스트
