...
# DATA-DESTINATIONS-OVERSEAS — 해외 여행지 정적 데이터(15개국 30개 도시 이상)

## Context
`DATA-DESTINATIONS-OVERSEAS`(해외 여행지 정적 데이터(15개국 30개 도시 이상))은 SCR-001 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-001
- Route: N/A
- Page Entry: `src/data/destinations.overseas.ts`
...
## Test Cases
- TC-1(Functional): 국내와 동일 필수 필드 + `country_code`
