...
## Context
`CMP-SCR003-FLIGHT-FORM`(항공 조건 입력·요약·외부이동)은 SCR-003(`/travel-tools`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-003
- Route: `/travel-tools`
- Page Entry: `src/components/scr003/FlightForm.tsx`
...
## Security/Privacy AC
- 서버 DB·로그·분석 미저장, 원시 입력값 미보존

## Test Cases
- TC-1(Functional): 국가/지역/출발일/귀국일 검증, 요약 표시, 외부이동(새 탭)
- TC-2(Visual): 비전달 고지 문구 고정 노출
- TC-3(Security/Privacy): 서버 DB·로그·분석 미저장, 원시 입력값 미보존
