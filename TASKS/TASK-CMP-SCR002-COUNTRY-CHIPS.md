...
# CMP-SCR002-COUNTRY-CHIPS — 방문 국가 Chip 목록(30개+)

## Context
`CMP-SCR002-COUNTRY-CHIPS`(방문 국가 Chip 목록(30개+))은 SCR-002(`/about`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-002
- Route: `/about`
- Page Entry: `src/components/scr002/CountryChips.tsx`

## Design Ref
- D-001 §8(Chip, 비활성 스타일)
...
## Test Cases
- TC-1(Functional): 대륙별 그룹, 30개 이상
- TC-2(Visual): Chip 컴포넌트 재사용
