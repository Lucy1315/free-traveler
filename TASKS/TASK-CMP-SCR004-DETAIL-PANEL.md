---
task_id: CMP-SCR004-DETAIL-PANEL
type: component
screen_id: SCR-004
route: "`/mates`"
page_entry: "`src/components/scr004/MateDetailPanel.tsx`"
depends_on:
  - API-MATES-ROUTES
  - SHARED-SHARE-UTIL
...
# CMP-SCR004-DETAIL-PANEL — 동행 상세(Desktop 분할/Mobile Drawer)

## Context
`CMP-SCR004-DETAIL-PANEL`(동행 상세(Desktop 분할/Mobile Drawer))은 SCR-004(`/mates`) 화면의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-004
- Route: `/mates`
- Page Entry: `src/components/scr004/MateDetailPanel.tsx`

## Design Ref
- D-001 §12(Drawer·Modal — Desktop 좌40:우60 분할)
...
## Functional AC
- 작성자 닉네임·조건·설명 표시, Desktop 좌40:우60 분할/Mobile 하단 Drawer
...
## Test Cases
- TC-1(Functional): 작성자 닉네임·조건·설명 표시, Desktop 좌40:우60 분할/Mobile 하단 Drawer
- TC-2(Visual): Drawer 스크림+포커스 트랩
- TC-3(Security/Privacy): 연락처(이메일·전화) 미노출
