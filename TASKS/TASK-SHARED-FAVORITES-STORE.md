...
# SHARED-FAVORITES-STORE — 즐겨찾기 localStorage 로직

## Context
`SHARED-FAVORITES-STORE`(즐겨찾기 localStorage 로직)은 SCR-001,005 영역의 구현 Task다. `docs/06_SRS_UIUX_REVISED.md`가 정의한 Requirement와 `design-reference/UI_CONTRACT.md`가 정의한 화면 계약을 실제 개발 가능한 단위로 좁힌 것이며, 이 문서 자체는 계획서로서 구현 코드를 포함하지 않는다.
...
- Screen: SCR-001,005
- Route: N/A
- Page Entry: `src/lib/favorites.ts`

## Design Ref
- D-001 §9(Destination Card 즐겨찾기 하트 아이콘)
...
## Security/Privacy AC
- 서버 미전송(localStorage 한정)

## Test Cases
- TC-1(Functional): 추가/해제/중복 방지, 조회
- TC-3(Security/Privacy): 서버 미전송(localStorage 한정)
- TC-4(Verify 연동): UNIT-MATE-STATE(재사용)로 위 시나리오를 실행해 통과를 확인한다.
...
- [ ] Verify 절의 검증 수단(UNIT-MATE-STATE(재사용))을 실행했거나 실행 계획이 명시돼 있다.
